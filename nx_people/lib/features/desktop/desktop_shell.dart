    ...
    return Scaffold(
      ...
      body: Stack(
        children: <Widget>[
          ...
          if (workspace.hasOverlay) _ResultOverlay(contextModel: workspace.overlayContext!),
        ],
      ),
    );
  }
    ...
    return DecoratedBox(
      ...
      child: Stack(
        children: <Widget>[
                  ...
                  onChanged: (value) {
                    if (value.trim().length > 1) {
                      ...
                      ref
                          ...
                          .showOverlay(repository.context('Search', value.trim()));
                    }
                  },
                ),
              ),
          ...
          if (workspace.createMenuOpen) const Positioned(right: 12, top: 40, child: _CreateMenu()),
        ],
      ),
    );
  }
...
class _PeoplePane extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(
      ...
      children: <Widget>[
        _PersonSection(title: 'Pinned', rows: repository.pinned()),
        _PersonSection(title: 'Recent', rows: repository.recent()),
        _PersonSection(title: 'Follow up', rows: repository.followUp()),
        _QuerySection(
          title: 'Companies',
          rows: [
            for (final company in companies)
              _QueryRow('Company', company, repository.count('Company', company)),
          ],
        ),
        _QuerySection(
          title: 'Meetings',
          rows: [
            for (final meeting in meetings)
              _QueryRow('Meeting', meeting, repository.count('Meeting', meeting)),
          ],
        ),
        _QuerySection(
          title: 'Planned',
          rows: [
            for (final item in planned)
              _QueryRow('Planned', item, repository.count('Planned', item)),
          ],
        ),
      ],
    );
          ...
          _QuerySection(
            ...
            rows: [
              for (final tag in system.tags)
                _QueryRow(system.name, tag, repository.count(system.name, tag)),
            ],
          ),
      ],
    );
...
class _PersonSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _SidebarSection(
      title: title,
      children: [
        for (final person in rows) _PersonSideLink(person: person),
      ],
    );
  }
}
...
class _PersonSideLink extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final active = ref.watch(peopleWorkspaceProvider).activePersonId == person.id;
    return _SideLink(
      active: active,
      leading: _StatusDot(color: _statusColor(person.statusColor)),
      label: person.name,
      onTap: () => ref.read(peopleWorkspaceProvider.notifier).openPerson(person.id),
    );
  }
}

class _QueryRow extends ConsumerWidget {
  const _QueryRow(this.type, this.label, this.count);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return _SideLink(
      ...
      onTap: () {
        ...
        ref
            .read(peopleWorkspaceProvider.notifier)
            .showOverlay(repository.context(type, label));
      },
    );
  }
}
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          TextButton.icon(
            onPressed: () =>
                ref.read(peopleWorkspaceProvider.notifier).returnToActiveContext(),
    ...
    return Column(
      ...
      children: <Widget>[
                ...
                children: <Widget>[
                  ...
                  Wrap(
                    ...
                    children: <Widget>[
                      ...
                      _InlineContextButton(type: 'Company', label: person.company),
                      Text(
                        ' · ${person.location}',
                        style: const TextStyle(
                          color: AppColors.muted,
                  ...
                  _PillWrap(
                    children: <Widget>[
                      ...
                      for (final tag in person.tags) _ContextPill(type: 'Tag', label: tag),
                    ],
                  ),
        ...
        _ProfileSection(
          ...
          child: Text(
            person.summary,
            style: const TextStyle(color: Color(0xff3f3f46), fontSize: 15, height: 1.65),
          ),
        ),
        _ProfileSection(
...
class _Inspector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final repository = ref.watch(peopleRepositoryProvider);
    final person = repository.byId(personId);
    return DecoratedBox(
      decoration: const BoxDecoration(
              ...
              children: <Widget>[
                _InspectorSection(
                  ...
                  children: <Widget>[
                    ...
                    _ContextPair(label: 'Company', type: 'Company', value: person.company),
                ...
                _InspectorSection(
                  ...
                  children: <Widget>[
                    ...
                    for (final item in person.planned)
                      _LinkLine(icon: Icons.radio_button_unchecked, type: 'Planned', label: item),
                    for (final id in person.relatedIds)
                      _RelatedPersonLink(person: repository.byId(id)),
                  ],
                ),
                _InspectorSection(
                  title: 'Tags',
...
class _ContextPair extends StatelessWidget {
  const _ContextPair({required this.label, required this.type, required this.value});
...
class _RelatedPersonLink extends ConsumerWidget {
  const _RelatedPersonLink({required this.person});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return TextButton.icon(
      onPressed: () => ref.read(peopleWorkspaceProvider.notifier).openPerson(person.id),
...
class _LinkLine extends ConsumerWidget {
  const _LinkLine({required this.icon, required this.type, required this.label});
