...
Extract reusable offline mechanics from Nx Notes without turning document
semantics into a generic framework. Migrate Nx Notes incrementally, validating
...
Gate: the original Nx Notes suite passed before production changes began.
...
### Phase 5: Nx Notes upload migration
...
Gate: focused upload tests and the complete Nx Notes suite pass.
...
Gate: no application referenced removed APIs; Nx Notes and nx_offline passed
...
- Updated Nx Notes and nx_offline documents to match the implemented code.
...
There are no server changes in this refactor. Nx Notes continues to use its
