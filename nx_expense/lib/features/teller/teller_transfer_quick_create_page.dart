...
import 'package:nx_expense/features/expense/widgets/date_attribute_picker_field.dart';
...
class _TellerTransferQuickCreateScreenState
  ...
  Future<void> _submit(ModelTypeView schema) async {
    ...
    try {
      final attrsMap = <String, dynamic>{amountKey: amt};
      String? dateKey;
      ...
      final id = await repo.upsert(upsert);
      await repo.linkTransferToTellerTimeline(
        transferId: id,
        tellerEventId: widget.row.eventId,
        tellerEventTime: widget.row.time,
      );
