...
Expands each section in [`nx-db-test-cases.md`](./nx-db-test-cases.md): **what** is under test, **inputs**, **expected** behavior. Intended to map into `mobile/nx_modules/nx_db/test/**` (and optionally app packages for `LoginPage`).
...
| PM7.4 | `modelProvider` filter shape | `filters` with `id` op `=` | Variables match [`models_provider`](../../lib/src/data_providers/models_provider.dart) |
