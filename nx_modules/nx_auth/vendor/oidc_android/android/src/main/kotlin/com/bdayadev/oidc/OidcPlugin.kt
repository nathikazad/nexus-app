...
class OidcPlugin :
    ...
    private fun launchAuthTab(
        ...
        val authTab = AuthTabIntent.Builder()
            ...
            .build()
        authTab.intent.setPackage(browserPackage)
    ...
    private fun onRedirect(data: Uri): Boolean {
        ...
        if (data.path != expected.path) return false
        if (data.getQueryParameter("state") != expectedState) return false
    ...
    private fun scheduleFlowTimeout(options: Map<String, Any?>) {
        ...
        mainHandler.postDelayed({
            if (pendingCallback != null && flowId == expectedFlowId) {
                ...
                finishPending(Result.failure(FlutterError("FLOW_TIMEOUT",
                    "Sign-in timed out. Return to the app and try again.", null)))
                activity?.let { host -> host.startActivity(Intent(host, host.javaClass).apply {
