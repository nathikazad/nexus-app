...
data class NativeStroke(val id: String, val points: List<InkPoint>, val color: Long, val width: Double) {
    // Strokes are immutable values: bounds are reusable across imports, erasers and rendering.
    val bounds:InkBounds? by lazy(LazyThreadSafetyMode.PUBLICATION) {
        if(points.isEmpty())null else InkBounds(points.minOf{it.x},points.minOf{it.y},points.maxOf{it.x},points.maxOf{it.y})
    }
}
...
object InkGeometry {
    ...
    internal fun clip(stroke: NativeStroke, contains: (InkPoint)->Boolean,
    ...
    fun disk(stroke: NativeStroke, center: InkPoint, radius: Double): List<NativeStroke> {
        ...
        if(stroke.bounds?.intersects(InkBounds(center.x-r,center.y-r,center.x+r,center.y+r))!=true)return listOf(stroke)
    ...
    fun region(stroke: NativeStroke, polygon: List<InkPoint>): List<NativeStroke> = InkRegion(polygon).erase(stroke)
...
class InkModel(var strokes: List<NativeStroke>, var view: InkViewport, var places: List<InkPlace>, var boards:InkBoards?=null) {
    ...
    fun eraseRegion(path: List<InkPoint>) {
        val region=InkRegion(path)
        replace(strokes.flatMap(region::erase))
    }
    ...
    fun select(path: List<InkPoint>) {
        ...
        val region=InkRegion(path)
        strokes.filter { region.erase(it)!=listOf(it) }.forEach { selected.add(it.id) }
    }
