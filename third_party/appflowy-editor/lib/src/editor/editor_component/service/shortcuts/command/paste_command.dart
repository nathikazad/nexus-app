...
CommandShortcutEventHandler _pasteCommandHandler = (editorState) {
  ...
  () async {
    ...
    if (text != null && text.isNotEmpty && text.looksLikeMarkdown) {
      await editorState.pasteMarkdown(text);
      return;
    }
...
final RegExp _markdownSyntaxRegex = RegExp(
  r'(^\s{0,3}#{1,6}\s+\S)|'
  r'(^\s{0,3}[-*+]\s+\S)|'
  r'(^\s{0,3}\d+[.)]\s+\S)|'
  r'(^\s{0,3}>\s+\S)|'
...
extension on EditorState {
  ...
  Future<void> pasteMarkdown(String markdown) async {
  ...
  Future<void> pastePlainText(String plainText) async {
    ...
    if (plainText.looksLikeMarkdown) {
      await pasteMarkdown(plainText);
      return;
    }
...
extension on String {
  bool get looksLikeMarkdown => _markdownSyntaxRegex.hasMatch(trim());
}
