...
class EpubReader {
  ...
  static Future<EpubBookRef> openBook(FutureOr<List<int>> bytes) async {
    ...
    // Bound eager parsing before inflating the individual entries.
    if (epubArchive.files.length > 20000 ||
        epubArchive.files.fold<int>(0, (sum, file) => sum + file.size) >
            128 * 1024 * 1024) {
      throw FormatException('EPUB exceeds the 128 MB expanded-content limit.');
    }
  ...
  static Future<EpubBook> readBook(FutureOr<List<int>> bytes,
      {bool decodeCover = true}) async {
    var result = EpubBook();
    List<int> loadedBytes;
    if (bytes is Future) {
      loadedBytes = await bytes;
    ...
    if (decodeCover) result.CoverImage = await epubBookRef.readCover();
