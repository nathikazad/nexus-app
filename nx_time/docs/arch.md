...
- **`features/`** — one folder per screen / vertical. Each contains the
  ...
  `package:nx_db/transcript.dart` (in practice, prefer consuming those
  through `data/providers.dart` re-exports to stay consistent).

### How `nx_time` relates to `nx_db`

`nx_db` ships **six** sub-libraries (plus the everything-shim). `nx_time` uses
each one from the right layer:
  ...
  lib/
    ...
    domain/                            # PURE DART
      action/
        ...
        action_subtype_option.dart      # { id, name } for subtypes / colors UI
      tasks/
        task.dart
    ...
    data/                              # KGQL + Riverpod ALLOWED HERE
      providers.dart                    # re-exports `package:nx_db/person` + `modelTypeColors` + repos
      action/
        ...
        action_subtypes_provider.dart   # subtypes (ModelType) + domain [ActionSubtypeOption] provider
      person/
        model_type_colors.dart          # kPrefModelTypeColors, merge helpers, [ModelTypeColors] + defaults (nx_time–specific; uses `package:nx_db/person` for `Person` / `PersonRepository`)
      tasks/
...
| A typed business object (Action, Task, …) | `domain/<e>/<e>.dart` (not `Person` — use `package:nx_db/person.dart`) | `domain/<e>/<e>_test.dart` |
