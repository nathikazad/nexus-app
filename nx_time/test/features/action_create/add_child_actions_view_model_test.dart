...
void main() {
  test(
    'parentActionForChildrenProvider loads parent with graph from fake repo',
    () async {
      final p = Action(
        id: 1,
        name: 'Parent',
      ...
      final container = makeContainer(
        overrides: [actionRepositoryProvider.overrideWith((ref) => fake)],
      );
      addTearDown(container.dispose);

      final key = (id: 1, modelTypeName: 'Goto');
      final loaded = await container.read(
        parentActionForChildrenProvider(key).future,
      );
