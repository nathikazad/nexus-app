...
void main() {
  ...
  test('lib/ top-level names match arch.md: core, data, domain, features + app shell', () {
    ...
    for (final entity in libRoot.listSync()) {
      ...
      if (entity is Directory) {
        expect(allowedDirs.contains(name), isTrue, reason: 'Unexpected directory in lib/: $name');
      } else if (entity is File && name.endsWith('.dart')) {
        expect(allowedFiles.contains(name), isTrue, reason: 'Unexpected file in lib/: $name');
      }
    }
  });
}
