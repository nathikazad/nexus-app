...
class _NodeForm {
  _NodeForm([String initial = '', this.id])
...
class _LogTagSystemFormPageState extends ConsumerState<LogTagSystemFormPage> {
  ...
  final List<_NodeForm> _flatNodes = [];
  ...
  void _ensureFromSchema(ModelTypeView schema) {
    ...
    if (ts.isHierarchical) {
      ...
      void flat(TagNodeView n) {
        _flatNodes.add(_NodeForm(n.name, n.id));
        for (final c in n.children ?? const <TagNodeView>[]) {
          flat(c);
        }
      }
      ...
      if (_flatNodes.isEmpty) _flatNodes.add(_NodeForm());
    }
    _didInit = true;
  }
    ...
    return Row(
      ...
      children: [
        ...
        if (_hierarchical)
          ...
          TextButton.icon(
            onPressed: () => setState(() => _flatNodes.add(_NodeForm())),
            style: TextButton.styleFrom(
              foregroundColor: AppColors.teal600,
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              minimumSize: Size.zero,
          ...
          children: [
            ...
            IconButton(
              ...
              onPressed: () {
                ...
                setState(() {
                  _markDeleted(_hierRoots[rootIndex]);
          ...
          child: Row(
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  if (_flatNodes.length <= 1) return;
                  setState(() {
                    _markDeleted(_flatNodes[i]);
                    _flatNodes[i].dispose();
                    _flatNodes.removeAt(i);
                  });
                },
              ),
            ],
  ...
  void _markDeleted(_NodeForm node) {
    final id = node.id;
    if (id != null) _deletedNodeIds.add(id);
  }

  List<SetTagNodeRequest> _deletedNodeRequests() {
    return [
      for (final id in _deletedNodeIds) SetTagNodeRequest(id: id, delete: true),
    ];
  }
  ...
  Future<void> _save(ModelTypeView schema) async {
    ...
    try {
      ...
      if (_hierarchical) {
        nodes = [
          ..._hierRoots.map((n) => n.toReq()),
          ..._deletedNodeRequests(),
        ];
      } else {
        nodes = _flatNodes.map((n) => n.toReq()).toList()
          ..addAll(_deletedNodeRequests());
      }
