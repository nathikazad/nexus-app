# Nx Notes native offline refactor
...
keeping all document semantics in Nx Notes. It preserves installed native
...
operation stale, Nx Notes fetches and imports the remote document and discards
...
Nx Notes owns its hash protocol:
...
Nx Notes still reads and writes the original preference keys and still names
...
```text
nx_notes/lib/application/native/
  background_uploader.dart
  document_outbox_adapter.dart
  native_document_session.dart
  native_notes_workspace.dart

nx_notes/lib/application/sync/document_synchronizer.dart
nx_notes/lib/data/local/drift/drift_local_notes_store.dart
nx_notes/lib/composition/notes_composition.dart
nx_notes/lib/features/shell/offline_sync_lifecycle.dart
...
for both `nx_offline` and `nx_notes`.
