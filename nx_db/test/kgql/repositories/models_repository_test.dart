...
void main() {
  ...
  group('setKgqlModelIfNewer', () {
    test('passes the immutable edit time and parses APPLIED', () async {
      final mock = MockGraphQLClient();
      MutationOptions? captured;
      when(() => mock.mutate(any())).thenAnswer((invocation) async {
        captured = invocation.positionalArguments[0] as MutationOptions;
        return QueryResult(
          options: MutationOptions(document: gql('mutation { __typename }')),
          source: QueryResultSource.network,
          data: {
      ...
      final editedAt = DateTime.utc(2026, 7, 27, 10, 30);

      final result = await setKgqlModelIfNewer(
        mock,
        SetModelRequest(id: 42, name: 'Latest'),
        clientUpdatedAt: editedAt,
      );

      expect(result.status, KgqlConditionalSaveStatus.applied);
      expect(result.modelId, 42);
      expect(result.updatedAt, editedAt);
      final input = captured!.variables['input'] as Map<String, dynamic>;
      expect(input['clientUpdatedAt'], editedAt.toIso8601String());
      expect((input['data'] as Map<String, dynamic>)['id'], 42);
    });
    ...
    test('parses STALE', () async {
      ...
      final result = await setKgqlModelIfNewer(
        mock,
        SetModelRequest(id: 7, name: 'Old'),
        clientUpdatedAt: DateTime.utc(2026, 7, 26),
      );

      expect(result.status, KgqlConditionalSaveStatus.stale);
      expect(result.modelId, 7);
      expect(result.updatedAt, DateTime.utc(2026, 7, 27, 11));
    });

    test('rejects create and delete requests locally', () async {
      final mock = MockGraphQLClient();

      expect(
        () => setKgqlModelIfNewer(
          mock,
          SetModelRequest(modelType: 'Document', name: 'New'),
      ...
      expect(
        () => setKgqlModelIfNewer(
          mock,
          SetModelRequest(id: 3, delete: true),
          clientUpdatedAt: DateTime.utc(2026),
        ),
        throwsArgumentError,
      );
