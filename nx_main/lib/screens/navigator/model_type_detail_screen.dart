...
class ModelTypeDetailScreen extends ConsumerWidget {
  const ModelTypeDetailScreen({super.key, required this.modelTypeId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(modelTypeProvider(modelTypeId));
  ...
  Widget _buildScaffold(BuildContext context, WidgetRef ref, ModelType modelType) {
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
          icon: const Icon(Icons.arrow_back),
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ref.invalidate(modelTypeProvider(modelTypeId));
          ref.invalidate(modelTypesProvider);
          await ref.read(modelTypeProvider(modelTypeId).future);
        },
        child: ListView(
  ...
  Widget _metaRow(BuildContext context, ModelType modelType) {
  ...
  Widget _traitTile(BuildContext context, ModelType trait) {
    return Padding(
  ...
  Widget _attributeCard(List<AttributeDefinition> attrs, {required bool muted}) {
  ...
  Widget _inheritedSection(BuildContext context, WidgetRef ref, ModelType modelType) {
    final parentId = modelType.parentId;
    if (parentId == null) return const SizedBox.shrink();

    final parentAsync = ref.watch(modelTypeProvider(parentId));
  ...
  Widget _relationCard(BuildContext context, WidgetRef ref, RelationshipType r) {
    final nameToId = ref.watch(modelTypeNameToIdProvider);
  ...
  Widget _tagSystemCard(TagSystem ts) {
