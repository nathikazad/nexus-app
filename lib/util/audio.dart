  ...
  Stream<Uint8List> bind(Stream<Uint8List> stream) {
    return stream.asyncExpand((opusPacket) async* {
      try {
        ...
        LoggingService.instance.log('Error in OpusToPcm16Transformer: $e');
    ...
    return stream.asyncExpand((pcm24Chunk) async* {
      ...
      while (buffer.length >= expectedPcm24Bytes) {
        ...
        if (resampled.length != expectedPcm16Bytes) {
          LoggingService.instance.log('[RESAMPLE] WARNING: Resampled chunk size ${resampled.length} != expected $expectedPcm16Bytes bytes');
          LoggingService.instance.log('[RESAMPLE] Input was ${chunk24.length} bytes (${chunk24.length ~/ 2} samples at 24kHz)');
          LoggingService.instance.log('[RESAMPLE] Output is ${resampled.length} bytes (${resampled.length ~/ 2} samples at 16kHz)');
        }
  ...
  Stream<Uint8List> bind(Stream<Uint8List> stream) {
    ...
    return encoder.bind(stream.map((pcm16Chunk) {
      ...
      if (pcm16Chunk.length != expectedPcm16Bytes) {
        LoggingService.instance.log('[ENCODE] ERROR: PCM16 chunk size ${pcm16Chunk.length} != expected $expectedPcm16Bytes bytes');
        LoggingService.instance.log('[ENCODE] Input was ${pcm16Chunk.length} bytes (${pcm16Chunk.length ~/ 2} samples)');
