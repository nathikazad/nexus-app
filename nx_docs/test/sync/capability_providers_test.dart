...
void main() {
  test('catalog refresh does not reopen a document session', () async {
    ...
    final workspace = FakeDocumentWorkspace(documents: <NxDocument>[document]);
    final container = ProviderContainer(
      overrides: [documentWorkspaceProvider.overrideWithValue(workspace)],
    );
    addTearDown(() async {
      container.dispose();
  ...
  test('session updates flow through without provider invalidation', () async {
    ...
    final workspace = FakeDocumentWorkspace(documents: <NxDocument>[original]);
    final container = ProviderContainer(
      overrides: [documentWorkspaceProvider.overrideWithValue(workspace)],
    );
    addTearDown(() async {
